...
use samael::idp::sp_extractor::{RequiredAttribute, SPMetadataExtractor};
use samael::idp::verified_request::UnverifiedAuthnRequest;
use samael::idp::{CertificateParams, IdentityProvider};
...
fn test_signed_response() {
    ...
    let out_xml = yaserde::ser::to_string(&out_response).expect("failed to serialize response xml");
...
fn test_signed_response_fingerprint() {
    ...
    let base64_cert = response
        ...
        .cloned()
        .unwrap();
    let der_cert = samael::crypto::decode_x509_cert(&base64_cert).expect("failed to decode cert ");
...
fn test_do_not_accept_unsigned_response() {
    ...
    // responses.
    let idp_metadata_xml = include_str!("../test_vectors/idp_metadata.xml");
    ...
    assert_eq!(
        sp.idp_metadata.idp_sso_descriptors[0].key_descriptors[0]
            .key_use
            .as_ref()
            .unwrap(),
        "signing"
    );
    assert!(!sp.idp_metadata.idp_sso_descriptors[0].key_descriptors[0]
        .key_info
        .x509_data
        .as_ref()
        .unwrap()
    ...
    let unsigned_response_xml = include_str!("../test_vectors/response.xml");
    ...
    assert!(matches!(
        resp.err().unwrap(),
        samael::service_provider::Error::FailedToParseSamlResponse
    ));
}
...
fn test_do_not_accept_signed_with_wrong_key() {
    ...
    let wrong_cert_signed_response_xml =
        include_str!("../test_vectors/response_signed_by_idp_2.xml");
    ...
    assert!(matches!(
        resp.err().unwrap(),
        samael::service_provider::Error::FailedToValidateSignature
    ));
}
...
fn test_accept_signed_with_correct_key_idp() {
    ...
    let wrong_cert_signed_response_xml = include_str!("../test_vectors/response_signed.xml");
...
fn test_accept_signed_with_correct_key_idp_2() {
    let idp_metadata_xml = include_str!("../test_vectors/idp_2_metadata.xml");

    let response_instant = "2014-07-17T01:01:48Z".parse::<DateTime<Utc>>().unwrap();
    let max_issue_delay = Utc::now() - response_instant + chrono::Duration::seconds(60);

    let sp = ServiceProvider {
        entity_id: "http://test_accept_signed_with_correct_key.test".to_string(),
        acs_url: Some("http://sp.example.com/demo1/index.php?acs".into()),
        idp_metadata: idp_metadata_xml.parse().unwrap(),
        max_issue_delay,
        ..Default::default()
